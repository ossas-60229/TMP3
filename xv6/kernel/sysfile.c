...
uint64 sys_open(void) {
    ...
    if(ip->type == T_SYMLINK){
        if(omode & O_NOFOLLOW){
            //printf("fucking O_NOFOLLOW %s\n",path);
        }else{
            int depth = 0;
            while(depth++ < MAX_DEPTH_LINK && ip->type == T_SYMLINK){
                uint64 len = 0, offset = 0;
                offset += readi(ip, 0, (uint64)&len, offset, sizeof(uint64));
                //printf("original path:%s\n",path);
                memset(path, 0, MAXPATH);
                offset += readi(ip, 0, (uint64)&path[0], offset, sizeof(char)*len);
                //path[len] = '\0';
                //printf("link to %s\n",path);
                iunlockput(ip);
                if((ip = namei(path)) == 0){
            ...
            if(depth >= MAX_DEPTH_LINK){
                //printf("fucking too deep\n");
                iunlockput(ip);
                end_op();
                return -1;
            }
...
uint64 sys_chdir(void) {
    ...
    ilock(ip);
    //printf("the fucking type is %d\n", ip->type);
    if(ip->type == T_SYMLINK){
        //printf("this is fucking symlink\n");
        int depth = 0;
        while(ip->type == T_SYMLINK && depth++ < MAX_DEPTH_LINK){
            memset(path, 0, MAXPATH);
            uint64 off = 0, len = 0;
            off += readi(ip, 0, (uint64)&len, off, sizeof(uint64));
            off += readi(ip, 0, (uint64)path, off, len);
            iunlockput(ip);
            if((ip = namei(path)) == 0){
                iunlockput(ip);
                end_op();
...
uint64 sys_symlink(void) {
    ...
    //printf("link %s to %s",path, target);
    begin_op();
    ip = namei(target);
    if (ip == 0) {
        //printf("fuck you 0\n");
        ...
        // if it's a symlink
        int depth = 0;
        //printf("fuck you start type is %d\n",ip->type);
        while(ip->type == T_SYMLINK && depth++ < MAX_DEPTH_LINK){
            uint64 offset = 0, len = 0;
            ilock(ip);
            offset += readi(ip, 0, (uint64)&len, offset, sizeof(uint64));
            memset(target, 0, MAXPATH);
            offset += readi(ip, 0, (uint64)target, offset, sizeof(char)*len);
            iunlockput(ip);
            ip = namei(target);
            if (ip == 0) {
                end_op();
    ...
    uint64 len = strlen(target);
    uint off = 0;
    //printf("path fucking start\n");
    iunlock(ip);
    ilock(ip);
    off += writei(ip, 0, (uint64)&len,off, sizeof(uint64));
    off += writei(ip, 0, (uint64)&target[0],off, len*sizeof(char));
    iupdate(ip);
    iunlockput(ip);
    //printf("path fucking end\n");
