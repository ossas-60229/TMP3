...
static struct inode *namex(char *path, int nameiparent, char *name) {
    ...
    while ((path = skipelem(path,name)) != 0) {
        ...
        if(ip->type == T_SYMLINK && strlen(path) > 0){
            ...
            uint64 len = 0, offt = 0;
            char tmp[MAXPATH];
            memset(tmp, 0, MAXPATH);
            begin_op();
            ilock(ip);
            offt += readi(ip, 0, (uint64)&len, 0, sizeof(uint64));
            offt += readi(ip, 0, (uint64)tmp, offt, len*sizeof(char));
            iunlockput(ip);
            end_op();
            //printf("tmp is %s\n", tmp);
            tmp[len++] = '/';
            memmove(&tmp[len], path, strlen(path));
            len += strlen(path);
            //printf("then tmp is %s\n", tmp);
            memmove(orgpath, &tmp[0], len);
            orgpath[len] = '\0';
            path = orgpath;
